...
public class EloraVersionedActionsBean implements Serializable {
    ...
    private void removeVersion(VersionModel selectedVersion)
        ...
        try {
            ...
            VersionRemoveCheckerManager versionRemoveCheckerManager = Framework.getService(
                    VersionRemoveCheckerManager.class);

            if (versionRemoveCheckerManager.canRemoveDocument(baseDoc, doc,
    ...
    private VersionModel getWorkingCopysPreviousVersion(DocumentModel doc)
        ...
        if (baseDoc.getId().equals(latestVersionDoc.getId())) {
            ...
            String majorVersion = baseDoc.getPropertyValue(
                    NuxeoMetadataConstants.NX_UID_MAJOR_VERSION).toString();
            String prevMajorVersion = previousDoc.getPropertyValue(
                    NuxeoMetadataConstants.NX_UID_MAJOR_VERSION).toString();
    ...
    public boolean isBaseVersion(String selectedVersionId)
        ...
        if (selectedVersion.getId().equals(baseDoc.getId())) {
            return true;
        } else {
            return false;
        }
