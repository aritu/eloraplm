...
function navigationInNewTabCB(url)
{
 if (url.indexOf("http")<0) {
   url= getBaseURL() + url;
 }
...
function navigateOnPopupDocInNewTab(docId)
{
    Seam.Component.getInstance("eloraPopupHelper").getNavigationURLOnPopupdocInNewTab(docId,navigationInNewTabCB);
}
...
function doOpenInNewTab(docid)
{
  navigateOnPopupDocInNewTab(docid);
}
