...
@Operation(id = UpdateThumbnails.ID, category = EloraGeneralConstants.OPERATIONS_CATEGORY_DEFAULT, label = "EloraImport - Update Thumbnails", description = "Update all the thumbnails of a document type.")
        ...
        try {
            ...
            if (doctype != null) {
                ...
                query += "select * from " + doctype
                        + " where ecm:mixinType != 'HiddenInNavigation' and ecm:primaryType = '"
                        + doctype + "' ";
            } else {
