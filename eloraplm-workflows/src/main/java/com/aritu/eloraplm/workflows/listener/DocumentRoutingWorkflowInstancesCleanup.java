...
public class DocumentRoutingWorkflowInstancesCleanup
    ...
    public void handleEvent(EventBundle events) {
        if (Framework.isBooleanPropertyTrue(
        ...
        DocumentRoutingService routing = Framework.getService(
                DocumentRoutingService.class);
        RepositoryManager repositoryManager = Framework.getService(
                RepositoryManager.class);

        Set<String> repositoryNames = new HashSet<>();
        for (Event event : events) {
            if (event.getContext().hasProperty(
                    CLEANUP_WORKFLOW_REPO_NAME_PROPERTY)) {
                String repositoryName = (String) event.getContext().getProperty(
                        CLEANUP_WORKFLOW_REPO_NAME_PROPERTY);
     ...
     * @since 7.1
     */
    private void doCleanAndReschedule(int batchSize,
            DocumentRoutingService routing, String repositoryName) {
        int cleanedUpWf = routing.doCleanupDoneAndCanceledRouteInstances(
                repositoryName, batchSize);
        if (cleanedUpWf == batchSize) {
            EventContextImpl eCtx = new EventContextImpl();
            eCtx.setProperty(CLEANUP_WORKFLOW_REPO_NAME_PROPERTY,
                    repositoryName);
            Event event = eCtx.newEvent(CLEANUP_WORKFLOW_EVENT_NAME);
            EventProducer eventProducer = Framework.getService(
                    EventProducer.class);
