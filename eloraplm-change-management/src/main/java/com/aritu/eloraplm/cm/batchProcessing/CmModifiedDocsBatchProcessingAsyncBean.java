...
@AutoCreate
@Name("cmModifiedDocsBatchProcessingAsync")
public class CmModifiedDocsBatchProcessingAsyncBean implements Serializable {
    private static final long serialVersionUID = 1L;

    private static final Log log = LogFactory.getLog(
            CmModifiedDocsBatchProcessingAsyncBean.class);
    ...
    @Observer(CMBatchProcessingEventNames.PROMOTE_MODIFIED_DOCS)
    public void promoteAsync(DocumentModel cmProcessDoc, TreeNode root,
        ...
        try {
            ...
            List<String> destinationWcUidList = processPromoteDocs(cmProcessDoc,
                    root, eloraDocumentRelationManager, session);
                    ...
                    try {
                        ...
                        if (PromoteHelper.checkReleasedInMajor(doc,
                            ...
                            log.trace(logInitMsg
                                    ...
                                    + docId + "|");
                            List<Statement> cadStmts = getCadHierarchicalStmts(
                                    doc,
                                    cadHierarchicalAndSpecialAndDirectRelations);
                            log.trace(logInitMsg
                                    + "Finished getting hierarchical children of document |"
                                    + docId + "|");
                            if (cadStmts == null || cadStmts.isEmpty()) {
                                promotable = true;
                                log.trace(logInitMsg
                                        + "cadStmts is null or empty");
                            }
                                ...
                                if (EloraDocumentHelper.isReleased(
                                    ...
                                    if (latestReleased != null) {
                                        ...
                                        log.error(logInitMsg
                                                ...
                                                + objectCadDoc.getId() + "|");
                                        // TODO::: hemen errorea sartzea falta
                                        // da????
                                        promotable = false;
                                    }
                                } else {
                            ...
                            log.error(logInitMsg + "Document  with docId = |"
